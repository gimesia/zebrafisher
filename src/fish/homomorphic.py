...
if __name__ == "__main__":
    ...
    image_path = r'C:\Users\gimesia\Documents\PROJEKT\zebrafish_pipenv\src\images\zf.jpg'
    ...
    img = cv2.normalize(img, None, alpha=0, beta=255,
                        norm_type=cv2.NORM_MINMAX)  # , alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
    show_img(img)
    homo_filter = HomomorphicFilter(a=0.85, b=3.0)
    img_filtered = homo_filter.filter(I=img, filter_params=[30, 2])
    show_img(img_filtered)
    cv2.imwrite(img=img_filtered, filename=filename)
